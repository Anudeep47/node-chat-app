...
io.on('connection', (socket) => {
  ...
  socket.on('join', (params, callback) => {
    if(!isRealString(params.name) || !isRealString(params.room)){
      return callback('Display name and Room name are incorrect');
    }
    ...
    socket.broadcast.to(params.room).emit('newMessage', generateMessage('Admin', `Yayy, ${params.name} has joined our Chat room :)`));
    return callback();
  });
